...
pub fn promote_single_root_dir(staging: &Path, final_dir: &Path) -> EnvrResult<()> {
    envr_platform::install_layout::commit_single_root_dir(
        staging,
        final_dir,
        nim_installation_valid,
        "empty nim archive",
        "expected exactly one root directory in nim archive",
        "expected nim archive root to be a directory",
        "extracted nim layout missing bin/nim",
    )
}
...
impl SpecDrivenInstaller for NimManager {
