...
fn download_to_path(
    ...
    for attempt in 1..=3 {
        ...
        if !status.is_success() {
            ...
            // Resume with Range can yield 416 when the local partial size does not match the
            // object (complete download, replaced release, etc.). Drop the partial and retry
            // without Range instead of failing permanently.
        ...
        // If we asked for a range but server ignored it and returned full 200, restart.
    ...
    Err(last_err.unwrap_or_else(|| {
        EnvrError::Download("ruby installer download failed (unknown error)".to_string())
    }))
}
