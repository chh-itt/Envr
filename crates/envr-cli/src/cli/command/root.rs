...
pub enum Command {
    ...
    Resolve {
        /// Language key: `node`, `python`, `java`, `kotlin`, …
        #[arg(value_name = "LANG")]
        lang: String,
    ...
    /// Run a subprocess with merged PATH for configured runtimes (e.g. node, python, java, kotlin) plus project `env`.
    ...
    Prune {
        /// Limit to one language (`node`, `python`, `java`, `kotlin`, …); default: all
        #[arg(value_name = "LANG")]
        lang: Option<String>,
