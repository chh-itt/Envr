...
"#,
    ...
    assert!(
        stdout.contains("ok") || stdout.contains("Installed missing pinned runtimes."),
        "expected run to execute or report install: {stdout}"
    );
}
...
"#,
    ...
    assert!(
        stderr.contains("node") || stderr.contains("99.99.99"),
        "expected missing pin details in human output: {stderr}"
    );
}
