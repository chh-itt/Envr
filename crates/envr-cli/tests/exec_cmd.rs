...
"#).expect("write envr toml");
    ...
    let out = Command::cargo_bin("envr")
        ...
        .args(["--format", "json", "exec", "--lang", "node", "--", "cmd", "/C", "echo", "ok"])
        .output()
        .expect("run");
    ...
    for key in [
        ...
        assert!(v["data"].get(key).is_some(), "expected exec json envelope to include {key}: {v}");
    }
}
...
"#).expect("write envr toml");
    ...
    for key in [
        ...
        assert!(v["data"].get(key).is_some(), "expected run json envelope to include {key}: {v}");
    }
}
