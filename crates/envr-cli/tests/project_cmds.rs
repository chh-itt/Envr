...
fn project_lock_creates_locked_file_and_sync_accepts_it() {
    ...
    assert!(
        lock_text.contains("[[runtime]]"),
        "unexpected lockfile:\n{lock_text}"
    );
    assert!(
        lock_text.contains("name = \"node\""),
        "unexpected lockfile:\n{lock_text}"
    );
    assert!(
        lock_text.contains("request = \"22.11.0\""),
        "unexpected lockfile:\n{lock_text}"
    );
    assert!(
        lock_text.contains("resolved = \"22.11.0\""),
        "unexpected lockfile:\n{lock_text}"
    );
    assert!(
        lock_text.contains("source = \"resolved\""),
        "unexpected lockfile:\n{lock_text}"
    );
    assert!(
        lock_text.contains("resolved_home"),
        "unexpected lockfile:\n{lock_text}"
    );
...
fn project_lock_dry_run_does_not_write_files() {
    ...
    assert!(
        !tmp.path().join(".envr.lock").exists(),
        "dry run should not create lockfile"
    );
    assert!(
        !tmp.path().join(".envr.lock.toml").exists(),
        "dry run should not create alt lockfile"
    );
}
...
fn project_sync_locked_json_reports_fresh_lock() {
    ...
    .expect("parse json");
    assert_eq!(
        v["data"]["lock_status"]["fresh"], true,
        "expected fresh lock status in json: {stdout}"
    );
    assert!(
        v["data"]["lock_status"]["path"].is_string(),
        "expected lock path in json: {stdout}"
    );
}
...
fn project_validate_locked_human_reports_lock_status() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("项目校验通过") || stdout.contains("project validation ok"),
        "expected human validate output to mention success: {stdout}"
    );
}
...
fn project_validate_json_reports_absent_or_stale_lock() {
    ...
    .expect("parse json");
    assert!(
        v["data"]["lock"].is_null() || v["data"]["lock"]["fresh"] == false,
        "expected unlocked lock status in json: {stdout}"
    );
}
...
fn project_validate_check_remote_reports_warnings() {
    ...
    .expect("parse json");
    assert!(
        v["data"]["remote_warnings"].is_array(),
        "expected remote warnings in json: {stdout}"
    );
}
...
fn project_validate_check_remote_human_reports_warning_hint() {
    ...
    assert!(
        stdout.contains("项目校验通过") || stdout.contains("project validation ok"),
        "expected human validation success output: {stdout}"
    );
}
