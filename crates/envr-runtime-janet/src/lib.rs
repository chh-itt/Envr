...
impl JanetRuntimeProvider {
    pub fn new() -> Self {
        Self {
            releases_api_url: DEFAULT_JANET_RELEASES_API_URL.to_string(),
    ...
    fn manager(&self) -> EnvrResult<JanetManager> {
        JanetManager::try_new(self.runtime_root()?, self.releases_api_url.clone())
    }
}

impl Default for JanetRuntimeProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl RuntimeProvider for JanetRuntimeProvider {
    fn kind(&self) -> RuntimeKind {
        RuntimeKind::Janet
    }

    fn list_installed(&self) -> EnvrResult<Vec<RuntimeVersion>> {
        list_installed_versions(&JanetPaths::new(self.runtime_root()?))
    }

    fn current(&self) -> EnvrResult<Option<RuntimeVersion>> {
        read_current(&JanetPaths::new(self.runtime_root()?))
    }
    ...
    fn uninstall_dry_run_targets(
        ...
        Ok((
            vec![JanetPaths::new(self.runtime_root()?).version_dir(&version.0)],
            None,
        ))
    }
}
