...
const JANET_RELEASES_ATOM_URL: &str = "https://github.com/janet-lang/janet/releases.atom";

static ATOM_RELEASE_TAG_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"https://github\.com/janet-lang/janet/releases/tag/([^"<>]+)"#)
        .expect("janet atom release tag regex")
});
static HTML_RELEASE_TAG_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"/janet-lang/janet/releases/tag/([^"<>/]+)"#).expect("janet html release tag regex")
});
...
pub fn blocking_http_client() -> EnvrResult<reqwest::blocking::Client> {
    reqwest::blocking::Client::builder()
        ...
        .user_agent(concat!("envr-runtime-janet/", env!("CARGO_PKG_VERSION")))
        .build()
    ...
    loop {
        ...
        let Some(arr) = v.as_array() else {
            return Err(EnvrError::Download(
                "janet releases API returned non-array payload".into(),
            ));
        };
        if arr.is_empty() {
            break;
...
fn fetch_rows_via_html(client: &reqwest::blocking::Client) -> EnvrResult<Vec<JanetInstallableRow>> {
    let mut out = Vec::new();
    let mut seen = HashSet::new();
    let mut empty_pages = 0usize;
    for page in 1..=30 {
        let url = format!("https://github.com/janet-lang/janet/releases?page={page}");
        ...
        for cap in HTML_RELEASE_TAG_RE.captures_iter(&text) {
            ...
            let Some(fname) = janet_primary_asset_filename(&version) else {
                continue;
            };
            let url = format!(
                "https://github.com/janet-lang/janet/releases/download/{tag}/{fname}"
            );
            out.push(JanetInstallableRow { version, url });
        }
        if found == 0 {
            empty_pages += 1;
            if empty_pages >= 2 {
...
fn fetch_rows_via_atom(client: &reqwest::blocking::Client) -> EnvrResult<Vec<JanetInstallableRow>> {
    let text = fetch_text(client, JANET_RELEASES_ATOM_URL)?;
    let mut out = Vec::new();
    let mut seen = HashSet::new();
    for cap in ATOM_RELEASE_TAG_RE.captures_iter(&text) {
        ...
        let url = format!("https://github.com/janet-lang/janet/releases/download/{tag}/{fname}");
        out.push(JanetInstallableRow { version, url });
    }
    out.sort_by(|a, b| cmp_release_labels(&a.version, &b.version));
    Ok(out)
...
pub fn list_remote_versions(rows: &[JanetInstallableRow], filter: &RemoteFilter) -> Vec<RuntimeVersion> {
