use crate::index::{
    JanetInstallableRow, blocking_http_client, fetch_janet_installable_rows_with_fallback,
    list_remote_latest_per_major_lines, list_remote_versions, resolve_janet_version,
};
...
impl JanetPaths {
    ...
    pub fn janet_home(&self) -> PathBuf {
        self.runtime_root.join("runtimes").join("janet")
    }
    pub fn versions_dir(&self) -> PathBuf {
        self.janet_home().join("versions")
    }
    pub fn current_link(&self) -> PathBuf {
        self.janet_home().join("current")
    }
    pub fn cache_dir(&self) -> PathBuf {
        self.runtime_root.join("cache").join("janet")
    }
...
pub fn janet_tool_candidate(home: &Path) -> Option<PathBuf> {
    first_existing(&[
        home.join("janet.exe"),
        home.join("janet"),
...
pub fn janet_installation_valid(home: &Path) -> bool {
    janet_tool_candidate(home).is_some()
}

pub fn list_installed_versions(paths: &JanetPaths) -> EnvrResult<Vec<RuntimeVersion>> {
...
pub fn read_current(paths: &JanetPaths) -> EnvrResult<Option<RuntimeVersion>> {
...
#[cfg(windows)]
fn copy_janet_windows_bin_payload(src_dir: &Path, bin_dir: &Path) -> EnvrResult<()> {
    fs::create_dir_all(bin_dir).map_err(EnvrError::from)?;
    for e in fs::read_dir(src_dir).map_err(EnvrError::from)? {
        let e = e.map_err(EnvrError::from)?;
        let p = e.path();
        if !p.is_file() {
            continue;
        ...
        let stem = p
            ...
            .unwrap_or("")
            .to_ascii_lowercase();
        if ext_lc == "dll" || stem == "janet" || stem == "jpm" {
            let dest = bin_dir.join(e.file_name());
            fs::copy(&p, &dest).map_err(EnvrError::from)?;
        }
    }
    Ok(())
}
...
fn install_from_msi_admin_unpack(msi: &Path, final_dir: &Path) -> EnvrResult<()> {
    ...
    let status = Command::new("msiexec")
        ...
        .map_err(|e| {
            EnvrError::Runtime(format!(
                "failed to spawn msiexec for janet MSI (is msiexec on PATH?): {e}"
            ))
        })?;
    if !status.success() {
    ...
    let janet_exe = find_named_executable_recursive(target, "janet").ok_or_else(|| {
        EnvrError::Validation(format!(
            "MSI unpack did not contain janet.exe under {}",
            target.display()
        ))
    })?;
    let src_dir = janet_exe.parent().ok_or_else(|| {
        EnvrError::Validation("janet.exe from MSI has no parent directory".into())
    })?;
    ...
    copy_janet_windows_bin_payload(src_dir, &bin_dir)?;
    if !janet_installation_valid(final_dir) {
        return Err(EnvrError::Validation(
            "janet MSI install produced no janet.exe under managed layout".into(),
        ));
    }
    Ok(())
}

fn promote_janet_extracted_tree(staging: &Path, final_dir: &Path) -> EnvrResult<()> {
    install_layout::ensure_final_parent(final_dir)?;
    let staging_final = install_layout::sibling_staging_path(final_dir)?;
    install_layout::remove_if_exists(&staging_final)?;

    if janet_installation_valid(staging) {
    ...
    if let (Some(root), true) = (first, only_one) {
        ...
        if root_path.is_dir() && janet_installation_valid(&root_path) {
    ...
    Err(EnvrError::Validation(
        "extracted janet layout missing janet executable".into(),
    ))
}

#[derive(Debug, Clone)]
pub struct JanetManager {
    paths: JanetPaths,
...
impl JanetManager {
    pub fn try_new(runtime_root: PathBuf, releases_api_url: String) -> EnvrResult<Self> {
        Ok(Self {
            paths: JanetPaths::new(runtime_root),
            releases_api_url,
        })
    }

    fn index_cache_ttl_secs() -> u64 {
        std::env::var("ENVR_JANET_RELEASES_CACHE_TTL_SECS")
            .ok()
            .or_else(|| std::env::var("ENVR_JANET_INDEX_CACHE_TTL_SECS").ok())
            .and_then(|s| s.trim().parse::<u64>().ok())
            .unwrap_or(3600)
    }

    fn latest_cache_ttl_secs() -> u64 {
        std::env::var("ENVR_JANET_REMOTE_CACHE_TTL_SECS")
            .ok()
            .and_then(|s| s.trim().parse::<u64>().ok())
            .unwrap_or(86400)
    }
    ...
    fn load_cached_rows(&self) -> Option<Vec<JanetInstallableRow>> {
        ...
        serde_json::from_str::<Vec<JanetInstallableRow>>(&text).ok()
    }
    ...
    fn fetch_rows(&self, force_refresh: bool) -> EnvrResult<Vec<JanetInstallableRow>> {
        if !force_refresh && let Some(rows) = self.load_cached_rows() {
            return Ok(rows);
        }
        let client = blocking_http_client()?;
        let rows = fetch_janet_installable_rows_with_fallback(&client, &self.releases_api_url)?;
    ...
    pub fn resolve_label(&self, spec: &str) -> EnvrResult<String> {
        let rows = self.fetch_rows(false)?;
        if let Some(v) = resolve_janet_version(&rows, spec) {
            return Ok(v);
        }
    ...
    pub fn set_current(&self, version: &RuntimeVersion) -> EnvrResult<()> {
        ...
        if !dir.is_dir() || !janet_installation_valid(&dir) {
            return Err(EnvrError::Validation(format!(
                "janet version not installed: {}",
                version.0
            )));
        }
