...
static HTML_RELEASE_TAG_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"/gleam-lang/gleam/releases/tag/([^"<>/]+)"#).expect("gleam html release tag regex")
});
...
pub fn blocking_http_client() -> EnvrResult<reqwest::blocking::Client> {
    reqwest::blocking::Client::builder()
        ...
        .user_agent(concat!("envr-runtime-gleam/", env!("CARGO_PKG_VERSION")))
        .build()
...
fn fetch_github_releases_index(
    ...
    loop {
        ...
        let Some(arr) = v.as_array() else {
            return Err(EnvrError::Download("gleam releases API returned non-array payload".into()));
        };
        if arr.is_empty() {
            break;
        }
...
pub fn list_remote_versions(rows: &[GleamInstallableRow], filter: &RemoteFilter) -> Vec<RuntimeVersion> {
