use crate::index::{
    GleamInstallableRow, blocking_http_client, fetch_gleam_installable_rows_with_fallback,
    list_remote_latest_per_major_lines, list_remote_versions, resolve_gleam_version,
};
...
impl GleamPaths {
    ...
    pub fn home(&self) -> PathBuf {
        self.runtime_root.join("runtimes").join("gleam")
    }
    ...
    pub fn cache_dir(&self) -> PathBuf {
        self.runtime_root.join("cache").join("gleam")
    }
...
pub fn gleam_tool_candidate(home: &Path) -> Option<PathBuf> {
    first_existing(&[
        home.join("gleam.exe"),
        home.join("gleam"),
...
pub fn gleam_installation_valid(home: &Path) -> bool {
    gleam_tool_candidate(home).is_some()
}
...
fn ensure_erlang_runtime_available() -> EnvrResult<()> {
    ...
    match cmd.output() {
        ...
        Ok(_) => Err(EnvrError::Runtime(
            "Erlang/OTP runtime check failed: `erl` is present but not runnable. Install or repair Erlang/OTP, then retry Gleam install.".into(),
        )),
        Err(_) => Err(EnvrError::Runtime(
            "missing Erlang/OTP runtime: `erl` (`erl.exe` on Windows) was not found on PATH. Install Erlang/OTP first, then retry Gleam install.".into(),
        )),
    }
}

fn gleam_candidate_dirs(staging: &Path) -> EnvrResult<Vec<PathBuf>> {
    let mut out = Vec::new();
    if staging.is_dir() {
...
fn pick_gleam_home_from_candidates(staging: &Path, candidates: &[PathBuf]) -> EnvrResult<PathBuf> {
    let mut val: Vec<PathBuf> = candidates
        .iter()
        .filter(|p| gleam_installation_valid(p))
        .cloned()
        .collect();
    match val.len() {
        0 => Err(EnvrError::Validation(
            "extracted gleam layout missing gleam executable".into(),
        )),
        1 => Ok(val[0].clone()),
        _ => {
...
fn promote_gleam_extract(staging: &Path, final_dir: &Path) -> EnvrResult<()> {
    ...
    let commit_if_valid = |home: &Path| -> EnvrResult<()> {
        if !gleam_installation_valid(home) {
    ...
    if gleam_installation_valid(staging) {
    ...
    let cands = gleam_candidate_dirs(staging)?;
    let inner = pick_gleam_home_from_candidates(staging, &cands)?;
    if inner == staging {
        fs::create_dir_all(&staging_final).map_err(EnvrError::from)?;
        install_layout::hoist_directory_children(staging, &staging_final)?;
        return commit_if_valid(&staging_final);
...
pub fn list_installed_versions(paths: &GleamPaths) -> EnvrResult<Vec<RuntimeVersion>> {
...
pub fn read_current(paths: &GleamPaths) -> EnvrResult<Option<RuntimeVersion>> {
...
impl GleamManager {
    pub fn try_new(runtime_root: PathBuf, releases_api_url: String) -> EnvrResult<Self> {
        Ok(Self {
            paths: GleamPaths::new(runtime_root),
            releases_api_url,
        })
    }
    fn index_cache_ttl_secs() -> u64 {
        std::env::var("ENVR_GLEAM_RELEASES_CACHE_TTL_SECS")
            .ok()
            .or_else(|| std::env::var("ENVR_GLEAM_INDEX_CACHE_TTL_SECS").ok())
            .and_then(|s| s.trim().parse::<u64>().ok())
            .unwrap_or(3600)
    }
    fn latest_cache_ttl_secs() -> u64 {
        std::env::var("ENVR_GLEAM_REMOTE_CACHE_TTL_SECS")
            .ok()
            .and_then(|s| s.trim().parse::<u64>().ok())
            .unwrap_or(86400)
    }
    fn load_cached_rows(&self) -> Option<Vec<GleamInstallableRow>> {
        ...
        serde_json::from_str::<Vec<GleamInstallableRow>>(&text).ok()
    }
    ...
    fn fetch_rows(&self, force_refresh: bool) -> EnvrResult<Vec<GleamInstallableRow>> {
        ...
        let rows = fetch_gleam_installable_rows_with_fallback(&client, &self.releases_api_url)?;
...
impl SpecDrivenInstaller for GleamManager {
