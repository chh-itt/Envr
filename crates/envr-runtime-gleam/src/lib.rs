...
pub struct GleamRuntimeProvider {
    releases_api_url: String,
    runtime_root_override: Option<PathBuf>,
}

impl GleamRuntimeProvider {
    pub fn new() -> Self {
        Self {
            releases_api_url: DEFAULT_GLEAM_RELEASES_API_URL.to_string(),
    ...
    fn manager(&self) -> EnvrResult<GleamManager> {
        GleamManager::try_new(self.runtime_root()?, self.releases_api_url.clone())
    }
}

impl Default for GleamRuntimeProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl RuntimeProvider for GleamRuntimeProvider {
    fn kind(&self) -> RuntimeKind {
        RuntimeKind::Gleam
    }
    fn list_installed(&self) -> EnvrResult<Vec<RuntimeVersion>> {
        list_installed_versions(&GleamPaths::new(self.runtime_root()?))
    }
    fn current(&self) -> EnvrResult<Option<RuntimeVersion>> {
        read_current(&GleamPaths::new(self.runtime_root()?))
    }
