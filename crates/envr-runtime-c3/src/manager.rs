use crate::index::{
    C3InstallableRow, blocking_http_client, fetch_c3_installable_rows_with_fallback,
    list_remote_latest_per_major_lines, list_remote_versions, resolve_c3_version,
};
...
impl C3Paths {
    ...
    pub fn c3_home(&self) -> PathBuf {
        self.runtime_root.join("runtimes").join("c3")
    }
    pub fn versions_dir(&self) -> PathBuf {
        self.c3_home().join("versions")
    }
    pub fn current_link(&self) -> PathBuf {
        self.c3_home().join("current")
    }
    pub fn cache_dir(&self) -> PathBuf {
        self.runtime_root.join("cache").join("c3")
    }
...
pub fn c3c_tool_candidate(home: &Path) -> Option<PathBuf> {
    first_existing(&[
        home.join("c3c.exe"),
        home.join("c3c"),
...
pub fn c3_installation_valid(home: &Path) -> bool {
    c3c_tool_candidate(home).is_some()
}

pub fn list_installed_versions(paths: &C3Paths) -> EnvrResult<Vec<RuntimeVersion>> {
...
pub fn read_current(paths: &C3Paths) -> EnvrResult<Option<RuntimeVersion>> {
...
fn promote_c3_extracted_tree(staging: &Path, final_dir: &Path) -> EnvrResult<()> {
    ...
    if c3_installation_valid(staging) {
    ...
    if let (Some(root), true) = (first, only_one) {
        ...
        if root_path.is_dir() && c3_installation_valid(&root_path) {
    ...
    Err(EnvrError::Validation(
        "extracted c3 layout missing c3c executable".into(),
    ))
}

#[derive(Debug, Clone)]
pub struct C3Manager {
    paths: C3Paths,
...
impl C3Manager {
    pub fn try_new(runtime_root: PathBuf, releases_api_url: String) -> EnvrResult<Self> {
        Ok(Self {
            paths: C3Paths::new(runtime_root),
            releases_api_url,
        })
    }

    fn index_cache_ttl_secs() -> u64 {
        std::env::var("ENVR_C3_RELEASES_CACHE_TTL_SECS")
            .ok()
            .or_else(|| std::env::var("ENVR_C3_INDEX_CACHE_TTL_SECS").ok())
            .and_then(|s| s.trim().parse::<u64>().ok())
            .unwrap_or(3600)
    }

    fn latest_cache_ttl_secs() -> u64 {
        std::env::var("ENVR_C3_REMOTE_CACHE_TTL_SECS")
            .ok()
            .and_then(|s| s.trim().parse::<u64>().ok())
            .unwrap_or(86400)
    }

    fn load_cached_rows(&self) -> Option<Vec<C3InstallableRow>> {
        ...
        serde_json::from_str::<Vec<C3InstallableRow>>(&text).ok()
    }

    fn save_cached_rows(&self, rows: &[C3InstallableRow]) -> EnvrResult<()> {
        fs::create_dir_all(self.paths.cache_dir()).map_err(EnvrError::from)?;
        let text =
            serde_json::to_string_pretty(rows)
                .map_err(|e| EnvrError::with_source(ErrorCode::Download, "serialize c3 rows cache", e))?;
        fs::write(self.paths.releases_cache_path(), text).map_err(EnvrError::from)?;
        let _ = fs::remove_file(self.paths.latest_cache_path());
        Ok(())
    }

    fn fetch_rows(&self, force_refresh: bool) -> EnvrResult<Vec<C3InstallableRow>> {
        if !force_refresh && let Some(rows) = self.load_cached_rows() {
            return Ok(rows);
        }
        let client = blocking_http_client()?;
        let rows = fetch_c3_installable_rows_with_fallback(&client, &self.releases_api_url)?;
    ...
    pub fn list_remote_latest_per_major(&self) -> EnvrResult<Vec<RuntimeVersion>> {
        ...
        let text =
            serde_json::to_string_pretty(&labels)
                .map_err(|e| EnvrError::with_source(ErrorCode::Download, "serialize c3 latest cache", e))?;
    ...
    pub fn resolve_label(&self, spec: &str) -> EnvrResult<String> {
        let rows = self.fetch_rows(false)?;
        if let Some(v) = resolve_c3_version(&rows, spec) {
            return Ok(v);
        }
        let rows = self.fetch_rows(true)?;
        resolve_c3_version(&rows, spec).ok_or_else(|| {
            EnvrError::Validation(format!("unknown c3 version spec: {spec}"))
        })
    }

    pub fn set_current(&self, version: &RuntimeVersion) -> EnvrResult<()> {
        let dir = self.paths.version_dir(&version.0);
        if !dir.is_dir() || !c3_installation_valid(&dir) {
            return Err(EnvrError::Validation(format!("c3 version not installed: {}", version.0)));
        }
...
impl SpecDrivenInstaller for C3Manager {
    fn install_from_spec(&self, request: &InstallRequest) -> EnvrResult<RuntimeVersion> {
        ...
        let row = rows
            ...
            .ok_or_else(|| EnvrError::Validation(format!("c3 version not found in index: {label}")))?;

        let final_dir = self.paths.version_dir(&label);
        if final_dir.is_dir() && c3_installation_valid(&final_dir) {
        ...
        let cache_file = self.paths.cache_dir().join(
            row.url.split('/').next_back().unwrap_or("c3-archive"),
        );
        ...
        promote_c3_extracted_tree(staging.path(), &final_dir)?;
        if !c3_installation_valid(&final_dir) {
            return Err(EnvrError::Validation("c3 install validation failed".into()));
        }
