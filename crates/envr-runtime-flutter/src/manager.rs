...
fn promote_single_root_dir(staging: &Path, final_dir: &Path) -> EnvrResult<()> {
    ...
    let first = iter
        ...
        .ok_or_else(|| EnvrError::Validation("empty flutter archive".into()))?;
    if iter.next().transpose().map_err(EnvrError::from)?.is_some() {
        return Err(EnvrError::Validation(
            "expected exactly one root directory in flutter archive".into(),
        ));
    }
    let inner = first.path();
    if !inner.is_dir() {
        return Err(EnvrError::Validation(
            "expected flutter archive root to be a directory".into(),
        ));
    }
...
impl FlutterManager {
    ...
    fn load_rows(&self) -> EnvrResult<Vec<FlutterIndexRow>> {
        ...
        let _ = (|| -> EnvrResult<()> {
            ...
            let s = serde_json::to_string(&rows).map_err(|e| EnvrError::Validation(e.to_string()))?;
