...
pub use index::{
    DEFAULT_ODIN_RELEASES_API_URL, GhAsset, GhRelease, OdinInstallableRow, blocking_http_client,
    fetch_odin_github_releases_index, installable_rows_from_releases,
    list_remote_latest_per_major_lines, list_remote_versions, odin_asset_prefix_candidates,
    resolve_odin_version,
};
pub use manager::{
    OdinManager, OdinPaths, list_installed_versions, odin_installation_valid, read_current,
};
...
pub struct OdinRuntimeProvider {
    releases_api_url: String,
    runtime_root_override: Option<PathBuf>,
}

impl OdinRuntimeProvider {
    pub fn new() -> Self {
        Self {
            releases_api_url: DEFAULT_ODIN_RELEASES_API_URL.to_string(),
    ...
    fn manager(&self) -> EnvrResult<OdinManager> {
        OdinManager::try_new(self.runtime_root()?, self.releases_api_url.clone())
    }
}

impl Default for OdinRuntimeProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl RuntimeProvider for OdinRuntimeProvider {
    fn kind(&self) -> RuntimeKind {
        RuntimeKind::Odin
    }

    fn list_installed(&self) -> EnvrResult<Vec<RuntimeVersion>> {
        list_installed_versions(&OdinPaths::new(self.runtime_root()?))
    }

    fn current(&self) -> EnvrResult<Option<RuntimeVersion>> {
        read_current(&OdinPaths::new(self.runtime_root()?))
    }
    ...
    fn uninstall_dry_run_targets(
        ...
        let p = OdinPaths::new(self.runtime_root()?).version_dir(&version.0);
        Ok((vec![p], None))
    }
}
