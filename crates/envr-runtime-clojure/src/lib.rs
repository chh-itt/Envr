//! Clojure CLI runtime (`clojure/brew-install` releases), `runtimes/clojure/versions/<label>/`.
...
pub use index::{
    blocking_http_client, fetch_clojure_github_releases_index, fetch_releases_json,
    installable_pairs_from_releases, list_remote_latest_per_major_lines, list_remote_versions,
    resolve_clojure_version,
};
pub use manager::{
    ClojureManager, ClojurePaths, clojure_installation_valid, clojure_tool_candidate,
    list_installed_versions, promote_clojure_extracted_tree, read_current,
};
pub use releases_url::{DEFAULT_CLOJURE_RELEASES_API_URL, resolved_clojure_releases_api_url};
...
pub struct ClojureRuntimeProvider {
    releases_api_url: String,
    runtime_root_override: Option<PathBuf>,
}

impl ClojureRuntimeProvider {
    pub fn new() -> Self {
        Self {
            releases_api_url: resolved_clojure_releases_api_url(),
    ...
    fn manager(&self) -> EnvrResult<ClojureManager> {
        ClojureManager::try_new(self.runtime_root()?, self.releases_api_url.clone())
    }
}

impl Default for ClojureRuntimeProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl RuntimeProvider for ClojureRuntimeProvider {
    fn kind(&self) -> RuntimeKind {
        RuntimeKind::Clojure
    }

    fn list_installed(&self) -> EnvrResult<Vec<RuntimeVersion>> {
        let paths = ClojurePaths::new(self.runtime_root()?);
    ...
    fn try_load_remote_latest_installable_per_major_from_disk(&self) -> Vec<RuntimeVersion> {
        ...
        let path = ClojurePaths::new(root)
            .cache_dir()
