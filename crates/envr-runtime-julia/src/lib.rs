//! Julia runtime: official `versions.json`, portable zip/tar.gz, `versions/<label>/` layout.

mod index;
mod manager;

pub use envr_platform::bin_tool_layout::julia_installation_valid;
pub use index::{
    DEFAULT_JULIA_VERSIONS_JSON_URL, blocking_http_client, fetch_versions_json,
    julia_cache_platform_tag, julia_host_target, list_remote_latest_per_major_lines,
    list_remote_versions, parse_versions_root, pick_file_for_host, resolve_julia_version,
};
pub use manager::{
    JuliaManager, JuliaPaths, list_installed_versions, promote_single_root_dir, read_current,
};
...
pub struct JuliaRuntimeProvider {
    versions_url: String,
    runtime_root_override: Option<PathBuf>,
}

impl JuliaRuntimeProvider {
    pub fn new() -> Self {
        Self {
            versions_url: DEFAULT_JULIA_VERSIONS_JSON_URL.to_string(),
    ...
    fn manager(&self) -> EnvrResult<JuliaManager> {
        JuliaManager::try_new(self.runtime_root()?, self.versions_url.clone())
    }

    fn remote_latest_per_major_cache_path(&self) -> EnvrResult<PathBuf> {
        let host = crate::julia_host_target()?;
        let tag = crate::julia_cache_platform_tag(host);
        let paths = JuliaPaths::new(self.runtime_root()?);
        Ok(paths
            .cache_dir()
    ...
    fn remote_cache_ttl_secs() -> u64 {
        ...
        std::env::var("ENVR_JULIA_REMOTE_CACHE_TTL_SECS")
            .ok()
...
impl Default for JuliaRuntimeProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl RuntimeProvider for JuliaRuntimeProvider {
    ...
    fn list_remote_latest_per_major(&self) -> EnvrResult<Vec<RuntimeVersion>> {
        ...
        let _ = (|| -> EnvrResult<()> {
            ...
            let s = serde_json::to_string(&strings).map_err(|e| {
                EnvrError::with_source(ErrorCode::Validation, "json encode julia latest labels", e)
            })?;
