...
pub fn promote_single_root_dir(staging: &Path, final_dir: &Path) -> EnvrResult<()> {
    envr_platform::install_layout::commit_single_root_dir(
        staging,
        final_dir,
        julia_installation_valid,
        "empty julia archive",
        "expected exactly one root directory in julia archive",
        "expected julia archive root to be a directory",
        "extracted julia layout missing bin/julia",
    )
}
