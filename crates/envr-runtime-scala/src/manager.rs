use crate::index::{
    GhRelease, blocking_http_client, fetch_scala_github_releases_index,
...
impl ScalaPaths {
    ...
    pub fn scala_home(&self) -> PathBuf {
        self.runtime_root.join("runtimes").join("scala")
    }

    pub fn versions_dir(&self) -> PathBuf {
        self.scala_home().join("versions")
    }

    pub fn current_link(&self) -> PathBuf {
        self.scala_home().join("current")
    }

    pub fn cache_dir(&self) -> PathBuf {
        self.runtime_root.join("cache").join("scala")
    }
...
pub fn scala_tool_candidate(home: &Path, stem: &str) -> Option<PathBuf> {
...
pub fn scala_installation_valid(home: &Path) -> bool {
    scala_tool_candidate(home, "scala").is_some() && scala_tool_candidate(home, "scalac").is_some()
}

fn find_scala_distribution_root(extract_root: &Path) -> Option<PathBuf> {
    if scala_installation_valid(extract_root) {
        return Some(extract_root.to_path_buf());
    }
    let entries = fs::read_dir(extract_root).ok()?;
    for e in entries.flatten() {
        let p = e.path();
        if p.is_dir() && scala_installation_valid(&p) {
            return Some(p);
        }
    }
    None
...
pub fn promote_scala_extracted_tree(staging: &Path, final_dir: &Path) -> EnvrResult<()> {
    let root = find_scala_distribution_root(staging).ok_or_else(|| {
        EnvrError::Validation(format!(
            "extracted scala layout missing scala/scalac under {}",
            staging.display()
        ))
    })?;
    ...
    if !scala_installation_valid(&staging_final) {
        let _ = fs::remove_dir_all(&staging_final);
        return Err(EnvrError::Validation(
            "promoted scala tree missing scala/scalac".into(),
        ));
    }
...
pub fn list_installed_versions(paths: &ScalaPaths) -> EnvrResult<Vec<RuntimeVersion>> {
...
pub fn read_current(paths: &ScalaPaths) -> EnvrResult<Option<RuntimeVersion>> {
...
fn ensure_java_preflight(runtime_root: &Path, scala_version_label: &str) -> EnvrResult<()> {
    ...
    if maj < 8 {
        return Err(EnvrError::Validation(format!(
            "Scala 需要 Java 8+。当前 JDK 目录名 `{label}` 推断主版本为 {maj}。请先安装并执行 `envr use java <版本>`。详见 docs/runtime/scala.md。"
        )));
    }
    if let Some(msg) =
        envr_domain::scala_java::scala_jdk_mismatch_message(scala_version_label, label)
    {
...
pub struct ScalaManager {
    pub paths: ScalaPaths,
...
impl ScalaManager {
    pub fn try_new(runtime_root: PathBuf, releases_api_url: String) -> EnvrResult<Self> {
        Ok(Self {
            paths: ScalaPaths::new(runtime_root),
            releases_api_url,
    ...
    fn index_ttl_secs() -> u64 {
        ...
        std::env::var("ENVR_SCALA_INDEX_CACHE_TTL_SECS")
            .ok()
    ...
    fn load_pairs(&self) -> EnvrResult<Vec<(String, String)>> {
        ...
        let releases = fetch_scala_github_releases_index(&self.client, &self.releases_api_url)?;
        let body = serde_json::to_string(&releases).map_err(|e| {
            EnvrError::with_source(ErrorCode::Validation, "scala releases json", e)
        })?;
        let _ = (|| -> EnvrResult<()> {
        ...
        if pairs.is_empty() {
            return Err(EnvrError::Validation(
                "scala: no installable scala3 archives in GitHub releases response for this platform"
                    .into(),
            ));
        }
        Ok(pairs)
    ...
    pub fn persist_remote_latest_per_major_cache(&self, list: &[RuntimeVersion]) -> EnvrResult<()> {
        ...
        let s = serde_json::to_string(&labels)
            .map_err(|e| EnvrError::with_source(ErrorCode::Validation, "json encode scala rows", e))?;
    ...
    pub fn resolve_label(&self, spec: &str) -> EnvrResult<String> {
        let pairs = self.load_pairs()?;
        resolve_scala_version(&pairs, spec)
    }
    ...
    pub fn install_resolved_version(
        ...
        promote_scala_extracted_tree(staging.path(), &final_dir)?;
    ...
    pub fn set_current(&self, version: &RuntimeVersion) -> EnvrResult<()> {
        ...
        if !scala_installation_valid(&dir) {
            return Err(EnvrError::Validation(format!(
                "scala {} is not installed under {}",
                version.0,
                dir.display()
            )));
        }
...
impl SpecDrivenInstaller for ScalaManager {
