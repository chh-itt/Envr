...
pub use manager::{
    BabashkaManager, BabashkaPaths, babashka_installation_valid, list_installed_versions,
    read_current,
};
...
pub struct BabashkaRuntimeProvider {
    releases_api_url: String,
    runtime_root_override: Option<PathBuf>,
}

impl BabashkaRuntimeProvider {
    pub fn new() -> Self {
        Self {
            releases_api_url: DEFAULT_BABASHKA_RELEASES_API_URL.to_string(),
    ...
    fn manager(&self) -> EnvrResult<BabashkaManager> {
        BabashkaManager::try_new(self.runtime_root()?, self.releases_api_url.clone())
    }
}

impl Default for BabashkaRuntimeProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl RuntimeProvider for BabashkaRuntimeProvider {
    fn kind(&self) -> RuntimeKind {
        RuntimeKind::Babashka
    }

    fn list_installed(&self) -> EnvrResult<Vec<RuntimeVersion>> {
        list_installed_versions(&BabashkaPaths::new(self.runtime_root()?))
    }

    fn current(&self) -> EnvrResult<Option<RuntimeVersion>> {
        read_current(&BabashkaPaths::new(self.runtime_root()?))
    }
    ...
    fn uninstall_dry_run_targets(
        ...
        Ok((
            vec![BabashkaPaths::new(self.runtime_root()?).version_dir(&version.0)],
            None,
        ))
    }
}
