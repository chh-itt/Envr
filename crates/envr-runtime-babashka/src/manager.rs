use crate::index::{
    BabashkaInstallableRow, blocking_http_client, fetch_babashka_installable_rows_with_fallback,
    list_remote_latest_per_major_lines, list_remote_versions, resolve_babashka_version,
};
...
impl BabashkaPaths {
    ...
    pub fn babashka_home(&self) -> PathBuf {
        self.runtime_root.join("runtimes").join("babashka")
    }
    pub fn versions_dir(&self) -> PathBuf {
        self.babashka_home().join("versions")
    }
    pub fn current_link(&self) -> PathBuf {
        self.babashka_home().join("current")
    }
    pub fn cache_dir(&self) -> PathBuf {
        self.runtime_root.join("cache").join("babashka")
    }
...
pub fn bb_tool_candidate(home: &Path) -> Option<PathBuf> {
    first_existing(&[
        home.join("bb.exe"),
        home.join("bb"),
        home.join("bin").join("bb.exe"),
        home.join("bin").join("bb"),
    ])
}

pub fn babashka_installation_valid(home: &Path) -> bool {
    bb_tool_candidate(home).is_some()
}

pub fn list_installed_versions(paths: &BabashkaPaths) -> EnvrResult<Vec<RuntimeVersion>> {
    ...
    for e in fs::read_dir(&dir).map_err(EnvrError::from)? {
        ...
        let p = e.path();
        if babashka_installation_valid(&p) {
...
pub fn read_current(paths: &BabashkaPaths) -> EnvrResult<Option<RuntimeVersion>> {
...
fn promote_bb_extracted_tree(staging: &Path, final_dir: &Path) -> EnvrResult<()> {
    ...
    if babashka_installation_valid(staging) {
    ...
    if let (Some(root), true) = (first, only_one) {
        ...
        if root_path.is_dir() && babashka_installation_valid(&root_path) {
    ...
    Err(EnvrError::Validation(
        "extracted babashka layout missing bb executable".into(),
    ))
}

#[derive(Debug, Clone)]
pub struct BabashkaManager {
    paths: BabashkaPaths,
    releases_api_url: String,
}

impl BabashkaManager {
    pub fn try_new(runtime_root: PathBuf, releases_api_url: String) -> EnvrResult<Self> {
        Ok(Self {
            paths: BabashkaPaths::new(runtime_root),
            releases_api_url,
        })
    }

    fn index_cache_ttl_secs() -> u64 {
        std::env::var("ENVR_BABASHKA_RELEASES_CACHE_TTL_SECS")
            .ok()
            .or_else(|| std::env::var("ENVR_BABASHKA_INDEX_CACHE_TTL_SECS").ok())
            .and_then(|s| s.trim().parse::<u64>().ok())
            .unwrap_or(3600)
    }

    fn latest_cache_ttl_secs() -> u64 {
        std::env::var("ENVR_BABASHKA_REMOTE_CACHE_TTL_SECS")
            .ok()
            .and_then(|s| s.trim().parse::<u64>().ok())
            .unwrap_or(86400)
    }

    fn load_cached_rows(&self) -> Option<Vec<BabashkaInstallableRow>> {
        ...
        serde_json::from_str::<Vec<BabashkaInstallableRow>>(&text).ok()
    }
    ...
    fn fetch_rows(&self, force_refresh: bool) -> EnvrResult<Vec<BabashkaInstallableRow>> {
        if !force_refresh && let Some(rows) = self.load_cached_rows() {
            return Ok(rows);
        }
        let client = blocking_http_client()?;
        let rows = fetch_babashka_installable_rows_with_fallback(&client, &self.releases_api_url)?;
    ...
    pub fn resolve_label(&self, spec: &str) -> EnvrResult<String> {
        let rows = self.fetch_rows(false)?;
        if let Some(v) = resolve_babashka_version(&rows, spec) {
            return Ok(v);
        }
        let rows = self.fetch_rows(true)?;
        resolve_babashka_version(&rows, spec)
            .ok_or_else(|| EnvrError::Validation(format!("unknown babashka version spec: {spec}")))
    }

    pub fn set_current(&self, version: &RuntimeVersion) -> EnvrResult<()> {
        let dir = self.paths.version_dir(&version.0);
        if !dir.is_dir() || !babashka_installation_valid(&dir) {
            return Err(EnvrError::Validation(format!(
                "babashka version not installed: {}",
                version.0
            )));
        }
...
impl SpecDrivenInstaller for BabashkaManager {
    fn install_from_spec(&self, request: &InstallRequest) -> EnvrResult<RuntimeVersion> {
        ...
        let row = rows.iter().find(|r| r.version == label).ok_or_else(|| {
            EnvrError::Validation(format!("babashka version not found in index: {label}"))
        })?;

        let final_dir = self.paths.version_dir(&label);
        if final_dir.is_dir() && babashka_installation_valid(&final_dir) {
        ...
        let cache_file = self
            .paths
            .cache_dir()
            .join(row.url.split('/').next_back().unwrap_or("babashka-archive"));
