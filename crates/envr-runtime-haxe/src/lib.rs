...
pub use index::{
    DEFAULT_HAXE_RELEASES_API_URL, HaxeInstallableRow, blocking_http_client,
...
pub struct HaxeRuntimeProvider {
    releases_api_url: String,
    runtime_root_override: Option<PathBuf>,
}

impl HaxeRuntimeProvider {
    pub fn new() -> Self {
        Self {
            releases_api_url: DEFAULT_HAXE_RELEASES_API_URL.to_string(),
    ...
    fn manager(&self) -> EnvrResult<HaxeManager> {
        HaxeManager::try_new(self.runtime_root()?, self.releases_api_url.clone())
    }
}

impl Default for HaxeRuntimeProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl RuntimeProvider for HaxeRuntimeProvider {
    fn kind(&self) -> RuntimeKind {
        RuntimeKind::Haxe
    }

    fn list_installed(&self) -> EnvrResult<Vec<RuntimeVersion>> {
        list_installed_versions(&HaxePaths::new(self.runtime_root()?))
    }

    fn current(&self) -> EnvrResult<Option<RuntimeVersion>> {
        read_current(&HaxePaths::new(self.runtime_root()?))
    }
