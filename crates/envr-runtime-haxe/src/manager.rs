use crate::index::{
    HaxeInstallableRow, blocking_http_client, fetch_haxe_installable_rows_with_fallback,
    list_remote_latest_per_major_lines, list_remote_versions, resolve_haxe_version,
};
...
impl HaxePaths {
    ...
    pub fn haxe_home(&self) -> PathBuf {
        self.runtime_root.join("runtimes").join("haxe")
    }
    pub fn versions_dir(&self) -> PathBuf {
        self.haxe_home().join("versions")
    }
    pub fn current_link(&self) -> PathBuf {
        self.haxe_home().join("current")
    }
    pub fn cache_dir(&self) -> PathBuf {
        self.runtime_root.join("cache").join("haxe")
    }
...
fn haxe_tool_candidate(home: &Path, stem: &str) -> Option<PathBuf> {
    let exe = format!("{stem}.exe");
    first_existing(&[
        home.join(&exe),
        home.join(stem),
        home.join("bin").join(&exe),
        home.join("bin").join(stem),
    ])
}

pub fn haxe_installation_valid(home: &Path) -> bool {
    let haxe = haxe_tool_candidate(home, "haxe");
    let haxelib = haxe_tool_candidate(home, "haxelib");
    let std = home.join("std");
    haxe.is_some() && haxelib.is_some() && std.is_dir()
}

pub fn list_installed_versions(paths: &HaxePaths) -> EnvrResult<Vec<RuntimeVersion>> {
...
pub fn read_current(paths: &HaxePaths) -> EnvrResult<Option<RuntimeVersion>> {
...
fn promote_haxe_extracted_tree(staging: &Path, final_dir: &Path) -> EnvrResult<()> {
    ...
    if haxe_installation_valid(staging) {
    ...
    if let (Some(root), true) = (first, only_one) {
        ...
        if root_path.is_dir() && haxe_installation_valid(&root_path) {
    ...
    Err(EnvrError::Validation(
        "extracted haxe layout missing haxe/haxelib or std directory".into(),
    ))
}

#[derive(Debug, Clone)]
pub struct HaxeManager {
    paths: HaxePaths,
...
impl HaxeManager {
    pub fn try_new(runtime_root: PathBuf, releases_api_url: String) -> EnvrResult<Self> {
        Ok(Self {
            paths: HaxePaths::new(runtime_root),
            releases_api_url,
        })
    }

    fn index_cache_ttl_secs() -> u64 {
        std::env::var("ENVR_HAXE_RELEASES_CACHE_TTL_SECS")
            .ok()
            .or_else(|| std::env::var("ENVR_HAXE_INDEX_CACHE_TTL_SECS").ok())
            .and_then(|s| s.trim().parse::<u64>().ok())
            .unwrap_or(3600)
    }

    fn latest_cache_ttl_secs() -> u64 {
        std::env::var("ENVR_HAXE_REMOTE_CACHE_TTL_SECS")
            .ok()
            .and_then(|s| s.trim().parse::<u64>().ok())
            .unwrap_or(86400)
    }
    ...
    fn load_cached_rows(&self) -> Option<Vec<HaxeInstallableRow>> {
        ...
        serde_json::from_str::<Vec<HaxeInstallableRow>>(&text).ok()
    }
    ...
    fn fetch_rows(&self, force_refresh: bool) -> EnvrResult<Vec<HaxeInstallableRow>> {
        if !force_refresh && let Some(rows) = self.load_cached_rows() {
            return Ok(rows);
        }
        let client = blocking_http_client()?;
        let rows = fetch_haxe_installable_rows_with_fallback(&client, &self.releases_api_url)?;
    ...
    pub fn resolve_label(&self, spec: &str) -> EnvrResult<String> {
        let rows = self.fetch_rows(false)?;
        if let Some(v) = resolve_haxe_version(&rows, spec) {
            return Ok(v);
        }
...
impl SpecDrivenInstaller for HaxeManager {
    fn install_from_spec(&self, request: &InstallRequest) -> EnvrResult<RuntimeVersion> {
        ...
        let cache_file = self
            .paths
            .cache_dir()
            .join(row.url.split('/').next_back().unwrap_or("haxe-archive"));
