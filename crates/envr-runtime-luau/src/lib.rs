...
pub use index::{
    DEFAULT_LUAU_RELEASES_API_URL, LuauInstallableRow, blocking_http_client,
    fetch_luau_installable_rows, list_remote_latest_per_major_lines, list_remote_versions,
    resolve_luau_version,
};
pub use manager::{
    LuauManager, LuauPaths, list_installed_versions, luau_analyze_tool_candidate,
    luau_installation_valid, luau_tool_candidate, read_current,
};
...
pub struct LuauRuntimeProvider {
    releases_api_url: String,
    runtime_root_override: Option<PathBuf>,
}

impl LuauRuntimeProvider {
    pub fn new() -> Self {
        Self {
            releases_api_url: DEFAULT_LUAU_RELEASES_API_URL.to_string(),
    ...
    fn manager(&self) -> EnvrResult<LuauManager> {
        LuauManager::try_new(self.runtime_root()?, self.releases_api_url.clone())
    }
}

impl Default for LuauRuntimeProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl RuntimeProvider for LuauRuntimeProvider {
    fn kind(&self) -> RuntimeKind {
        RuntimeKind::Luau
    }

    fn list_installed(&self) -> EnvrResult<Vec<RuntimeVersion>> {
        list_installed_versions(&LuauPaths::new(self.runtime_root()?))
    }

    fn current(&self) -> EnvrResult<Option<RuntimeVersion>> {
        read_current(&LuauPaths::new(self.runtime_root()?))
    }
    ...
    fn uninstall_dry_run_targets(
        ...
        let paths = LuauPaths::new(self.runtime_root()?);
