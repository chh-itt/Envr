...
### T910 i18n 强制规范
- [x] **T910：建立 i18n 强制规范（除专业术语外全部国际化）** #i18n #gui #cli
  ...
  - **进度**：done
  - **实现记录**：
    - 实现要点：新增 `docs/i18n/style-guide.md`，定义 i18n 强制范围（GUI+CLI 用户可见文本）、key 命名规范、插值与复数规则、缺失 key 回退策略、开发 DoD 与评审检查清单，并明确与 T911~T915 的衔接。
    - 相关提交/PR：
    - 遇到的问题/决策：将“专业术语可保留原文”定义为受控例外，要求后续纳入术语表/白名单（T913）统一治理。
    - 验收结果：规范文档已可直接用于开发与评审执行，后续可由 T914 自动化为 CI 检查规则。

### T911 GUI 文本全量 i18n
- [x] **T911：GUI 所有可见文本迁移为 i18n key** #i18n #gui
