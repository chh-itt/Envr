...
These files describe **machine-readable** CLI output when `envr` is run with `--format json` (or `ENVR_OUTPUT_FORMAT=json`).
...
| `code` = `project_check_failed` | [data-project-check-failed-v1.schema.json](./data-project-check-failed-v1.schema.json) |
