...
- **Windows `.msi` with no portable zip** (example: Janet `janet-{semver}-windows-x64-installer.msi`): plan for **`msiexec /a` administrative unpack** to a staging `TARGETDIR`, then **discover** `janet.exe` (layout varies), copy **`janet.exe` / `jpm.exe` (if present) / `*.dll`** into `runtimes/<key>/versions/<label>/bin/`, validate with `janet --version`, and document elevation/network policy failures.
...
- **Windows batch quoting trap**: `elixir.bat` uses `"%ERTS_BIN%erl.exe"`. When `ERTS_BIN` is empty, it becomes `"erl.exe"` and `cmd` will not resolve it via `PATH`. Mitigations used in envr: (1) shim resolution injects `ERTS_BIN` / `ERLANG_HOME` from the envr-managed `erlang/current` home when Elixir PATH proxy is on; (2) post-extract patch of `elixir.bat` so delayed-expansion startup can set `ERTS_BIN` from `%ERLANG_HOME%\\bin\\` (match both LF and CRLF in the upstream file); (3) install-time validation runs `elixir --version` with those env vars set from `runtimes/erlang/current`. **Erlang preflight on Windows**: prefer `cmd /C "erl -noshell -eval halt()."` over spawning `erl.exe` directly, because users often have only `erl.cmd` shims on `PATH`.
