...
- If API discovery fails (including GitHub 403), envr falls back to GitHub releases HTML pagination (tag extraction) and then to `https://github.com/purescript/purescript/releases.atom`, both with synthetic download URLs.
...
`envr use purescript 0.15` resolves to the latest installed/remote matching `0.15.x` label.
